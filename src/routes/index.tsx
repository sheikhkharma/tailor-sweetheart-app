...
const stats = [
  ...
  },
  {
    ...
    value: mockCommandes
      ...
      .padStart(2, "0"),
    icon: ClipboardList,
    detail: `${mockCommandes.filter((c) => c.statut === "essayage").length} prêtes pour essayage`,
  },
  {
    label: "Terminées",
    value: mockCommandes
...
function Dashboard() {
  const recentOrders = mockCommandes.slice(0, 5);
    ...
    <div className="max-w-7xl mx-auto px-6 py-12 space-y-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 animate-slide-up" style={{ animationDelay: "100ms" }}>
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="bg-card ring-1 ring-border p-6 space-y-3"
          >
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 lg:col-span-8 animate-slide-up" style={{ animationDelay: "200ms" }}>
          <div className="flex items-center justify-between mb-6 border-b border-border pb-4">
            ...
            <Link to="/commandes" className="text-xs font-bold text-primary uppercase tracking-widest hover:underline">
              Voir tout →
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {recentOrders.map((cmd) => {
              ...
              return (
                <div
                  key={cmd.id}
                  className="bg-card ring-1 ring-border p-5 flex items-center justify-between gap-4"
                >
        ...
        <div className="col-span-12 lg:col-span-4 animate-slide-up" style={{ animationDelay: "300ms" }}>
          <div className="flex items-center justify-between mb-6 border-b border-border pb-4">
            ...
            <Link to="/clients" className="text-xs font-bold text-primary uppercase tracking-widest hover:underline">
              Voir tout →
            </Link>
          </div>
          ...
          <div className="space-y-3">
            {mockClients.slice(0, 5).map((client) => (
              <Link
                ...
                className="block bg-card ring-1 ring-border p-4 hover:ring-primary/30 transition-all"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {getCategoryLabel(client.categorie)} • {client.telephone}
                    </p>
                  </div>
                </div>
              </Link>
